 ...
 * <ul>
 * <li>{@link #canServe(String)} method to test URI scheme this file system implementation can serve (see {@link FileSystemFactory})</li> 
...
public abstract class AbstractFileSystem implements FileSystemInterface {
	...
	@Override
	public FileSystemInterface open(String path) throws IOException {
	...
	@Override
	public FileSystemInterface push(String path) throws IOException {
		if (path == null || path.isEmpty()) {
			throw new IllegalArgumentException("Path can't be null or empty");
		}
		else {
	...
	public String[] list() throws IOException {
		...
		list(fsys -> result.add(fsys.getPath()));
	...
	public FileSystemInterface mount(final FileSystemInterface another) throws IOException {
		if (another == null) {
			throw new IllegalArgumentException("Another file system can't be null");
		}
	...
	private URI build(final String delta) throws IOException {
		...
		final String[]		newPart = delta.toString().split("\\/");
	...
	private FileSystemInterface open(URI item) {
	...
	private Redirection needRedirect(final URI address) {
		...
		else if (address.compareTo(ROOT_URI) == 0) {
			return null;
		}
