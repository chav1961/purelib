...
loop:   while (true) {
            switch (c = readUnsignedByte()) {
              ...
              default:
                if (--room < 0) {
                    buf = new char[offset + INITIAL_LINE_BUFFER_SIZE];
	...
	public String readUTF() throws IOException {
        ...
        if (utflen > 0) {
        	final byte[]	tempb = bytearr;
        	final char[]	tempc = chararr;
            int c, char2, char3;
            int count = 0;
        	
	        readFully(tempb, 0, utflen);
	        while (count < utflen) {
	            c = (int) tempb[count] & 0xFF;
	            if (c > 127) break;
	            count++;
	            tempc[chararr_count++]=(char)c;
	        }
	        ...
	        while (count < utflen) {
	            ...
	            switch (c >> 4) {
	                ...
	                case 12: case 13:
	                    ...
	                    char2 = (int) tempb[count-1];
	                    if ((char2 & 0xC0) != 0x80)
	                        throw new UTFDataFormatException("malformed input around byte " + count);
	                    tempc[chararr_count++]=(char)(((c & 0x1F) << 6) |
	                                                    (char2 & 0x3F));
	                    break;
	                case 14:
	                    ...
	                    if (count > utflen)
	                        throw new UTFDataFormatException("malformed input: partial character at end");
	                    char2 = (int) tempb[count-2];
	                    char3 = (int) tempb[count-1];
	                    if (((char2 & 0xC0) != 0x80) || ((char3 & 0xC0) != 0x80))
	                        throw new UTFDataFormatException("malformed input around byte " + (count-1));
	                    tempc[chararr_count++]=(char)(((c     & 0x0F) << 12) |
	                                                    ((char2 & 0x3F) << 6)  |
	                                                    ((char3 & 0x3F) << 0));
	                    break;
	                default:
