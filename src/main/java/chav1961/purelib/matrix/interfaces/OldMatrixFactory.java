 ...
 * @since 0.0.7
 */
public interface OldMatrixFactory {
	/**
	...
	public final static class Factory {
		...
		public static boolean canServe(final URI matrixFactoryUri) throws IllegalArgumentException, PreparationException {
			if (matrixFactoryUri == null) {
				...
				for (OldMatrixFactory item : ServiceLoader.load(OldMatrixFactory.class)) {
					if ((item instanceof SpiService) && ((SpiService<OldMatrixFactory>)item).canServe(matrixFactoryUri)) {
						return true;
					}
				}				
				return false;
		 ...
		 * @param matrixFactoryUri matrix factory URI to get matrix factory for. Can't be null and must have scheme {@value OldMatrixFactory#MATRIX_FACTORY_SCHEME}
		 * @return matrix factory found
		 * @throws IllegalArgumentException when localizer URI is null or doesn't have {@value Localizer#LOCALIZER_SCHEME} scheme
		 * @throws LocalizationException on any errors on creation localizer
		 */
		public static OldMatrixFactory newInstance(final URI matrixFactoryUri) throws IllegalArgumentException, PreparationException {
			if (matrixFactoryUri == null || !OldMatrixFactory.MATRIX_FACTORY_SCHEME.equals(matrixFactoryUri.getScheme())) {
				throw new IllegalArgumentException("Matrix URI can't be null and must have scheme ["+OldMatrixFactory.MATRIX_FACTORY_SCHEME+"]"); 
			}
			else {
				for (OldMatrixFactory item : ServiceLoader.load(OldMatrixFactory.class)) {
					if ((item instanceof SpiService) && ((SpiService<OldMatrixFactory>)item).canServe(matrixFactoryUri)) {
						try {
							return ((SpiService<OldMatrixFactory>)item).newInstance(matrixFactoryUri);
