...
class CFMatrixImpl implements Matrix {
	private final int		rows;
	private final int		cols;
	private final float[]	content;

	CFMatrixImpl(final int rows, final int cols) {
		this.rows = rows;
		this.cols = cols;
		this.content = new float[2 * rows * cols];
	}
	...
	public int[] extractInts() {
		throw new UnsupportedOperationException("Can't extract ints because matrix content is complex float"); 
	}
	...
	public long[] extractLongs() {
		throw new UnsupportedOperationException("Can't extract longs because matrix content is complex float"); 
	}
	...
	public float[] extractFloats(final Piece piece) {
		if (piece == null) {
			...
			System.arraycopy(content, 2 * (piece.getTop() * numberOfColumns() + piece.getLeft()), result, 0, result.length);
			return result;
		}
		else {
			final float[]	result = new float[2 * piece.getWidth() * piece.getHeight()];
			final int		targetWidth = 2 * piece.getWidth();

			MatrixUtils.ensurePieceIsValid(piece, numberOfRows(), numberOfColumns());
			for(int x = piece.getTop(), maxX = piece.getTop() + piece.getHeight(), to = 0; x < maxX; x++, to += targetWidth) {
				System.arraycopy(content, 2 * (x * numberOfColumns() + piece.getLeft()), result, to, targetWidth);
			}
			return result;
		}
	}
	...
	public double[] extractDoubles() {
		throw new UnsupportedOperationException("Can't extract doubles because matrix content is complex float"); 
	}
	...
	public Matrix assign(final int... content) {
		throw new UnsupportedOperationException("Can't assign ints because matrix content is complex float"); 
	}
	...
	public Matrix assign(final long... content) {
		throw new UnsupportedOperationException("Can't assign longs because matrix content is complex float"); 
	}
	...
	public Matrix assign(final Piece piece, final float... content) {
		if (piece == null) {
			...
			if (2 * piece.getWidth() * piece.getHeight() != content.length) {
				...
				System.arraycopy(content, 0, this.content, 2 * (piece.getTop() * numberOfColumns() + piece.getLeft()), content.length);
				return this;
			}
		}
		else {
			...
			if (2 * piece.getWidth() * piece.getHeight() != content.length) {
				...
				for(int x = piece.getTop(), maxX = piece.getTop() + piece.getHeight(), from = 0; x < maxX; x++, from += targetWidth) {
					System.arraycopy(content, from, this.content, 2 * (x * numberOfColumns() + piece.getLeft()), targetWidth);
				}
				return this;
			}
		}
	...
	public Matrix assign(final double... content) {
		throw new UnsupportedOperationException("Can't assign doubles because matrix content is complex float"); 
	}
	...
	public Matrix assign(final Matrix content) {
		if (content == null) {
			...
			System.arraycopy(content.extractFloats(), 0, this.content, 0, this.content.length);
			return this;
		}
	}
	...
	public Matrix fill(final int value) {
		throw new UnsupportedOperationException("Can't fill ints because matrix content is complex float"); 
	}
	...
	public Matrix fill(final long value) {
		throw new UnsupportedOperationException("Can't fill longs because matrix content is complex float"); 
	}
	...
	public Matrix fill(final float value) {
		throw new UnsupportedOperationException("Can't fill real floats because matrix content is complex float"); 
	}

	@Override
	public Matrix fill(final Piece piece, final float value) {
		throw new UnsupportedOperationException("Can't fill floats because matrix content is complex float"); 
	}

	@Override
	public Matrix fill(final float real, final float image) {
		final float[]	temp = this.content;
		...
		if (piece == null) {
			...
			for(int x = piece.getTop(), maxX = piece.getTop() + piece.getHeight(); x < maxX; x++) {
				for(int y = piece.getLeft(), maxY = piece.getLeft() + piece.getWidth(); y < maxY; y++) {
					this.content[2 * (x * numberOfColumns() + y)] = real;
					this.content[2 * (x * numberOfColumns() + y) + 1] = image;
				}
			}
			return this;
		}
	...
	public Matrix fill(final double value) {
		throw new UnsupportedOperationException("Can't fill doubles because matrix content is complex float"); 
	}
	...
	public Matrix add(final int... content) {
		throw new UnsupportedOperationException("Can't add ints because matrix content is complex float"); 
	}

	@Override
	public Matrix add(final long... content) {
		throw new UnsupportedOperationException("Can't add longs because matrix content is complex float"); 
	}
	...
	public Matrix add(final double... content) {
		throw new UnsupportedOperationException("Can't add doubles because matrix content is complex float"); 
	}
	...
	public Matrix addValue(final long value) {
		throw new UnsupportedOperationException("Can't add long because matrix content is complex float"); 
	}
	...
	public Matrix subtract(final int... content) {
		throw new UnsupportedOperationException("Can't subtract ints because matrix content is complex float"); 
	}

	@Override
	public Matrix subtract(final long... content) {
		throw new UnsupportedOperationException("Can't subtract longs because matrix content is complex float"); 
	}
	...
	public Matrix subtract(final double... content) {
		throw new UnsupportedOperationException("Can't subtract doubles because matrix content is complex float"); 
	}
	...
	public Matrix mulValue(final int value) {
		throw new UnsupportedOperationException("Can't multiply ints because matrix content is complex float"); 
	}

	@Override
	public Matrix mulValue(final long value) {
		throw new UnsupportedOperationException("Can't multiply longs because matrix content is complex float"); 
	}
	...
	public Matrix mulValue(final double value) {
		throw new UnsupportedOperationException("Can't multiply doubles because matrix content is complex float"); 
	}
	...
	public Matrix divValue(final int value) {
		throw new UnsupportedOperationException("Can't divide ints because matrix content is complex float"); 
	}

	@Override
	public Matrix divValue(final long value) {
		throw new UnsupportedOperationException("Can't divide longs because matrix content is complex float"); 
	}
	...
	public Matrix divValue(final double value) {
		throw new UnsupportedOperationException("Can't divide doubles because matrix content is complex float"); 
	}
	...
	public Matrix tensorMul(final Matrix content) {
		...
		if (content == null) {
			...
			final Matrix	result = new CFMatrixImpl(rowSize, colSize);
			final float[]	left = this.content, right = content.extractFloats(), target = result.extractFloats();
			
			for(int xLeft = 0, maxXLeft = numberOfRows(); xLeft < maxXLeft; xLeft++) {
				for(int yLeft = 0, maxYLeft = numberOfColumns(); yLeft < maxYLeft; yLeft++) {
					final float	multiplier = left[xLeft * numberOfColumns() + yLeft];
		...
		if (dir == null) {
			...
			switch (dir) {
				case ByColumns	:
					...
					amount = numberOfRows();
					result = new CFMatrixImpl(1, numberOfColumns());
					break;
				case ByRows		:
					loops = numberOfRows();
					step = 1;
					...
					result = new CFMatrixImpl(numberOfRows(), 1);
					break;
				default:
	...
	public String toHumanReadableString() {
		...
		for(int x = 0, maxX = numberOfRows(); x < maxX; x++) {
			...
			for(int y = 0, maxY = numberOfColumns(); y < maxY; y++) {
				sb.append(String.format("%1$15g+%1$15gi", this.content[2 * (x * maxY + y)], this.content[2 * (x * maxY + y) + 1]));
			}
			sb.append('\n');
		}
		return sb.toString();
