 ...
 * This class is designed to use with the {@linkplain FileSystemInterface} data sources and extends ability to keep classes in the different
 * sources</p>
 * <p>This class loader is a collection of <i>internal</i> class loaders, every of this loads it's own plugin. The overridden 
 ...
 * </code></p>
 * <p><b>WARNING!</b> Don't use {@link Class#forName(String, boolean, ClassLoader)} method to load classes from this loader, because
					...
					if (fs.exists() && fs.isFile() && fs.canRead()) {
						try(final InOutGrowableByteArray	gba = new InOutGrowableByteArray(false)) {
							try(final InputStream			is = fs.read()) {
								...
								final byte[]				definition = gba.extract();
								final Class<?>				cl = defineClass(className,definition,0,definition.length);
