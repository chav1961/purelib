...
public class GrowableShortArray {
	...
	private final AbstractArrayContentManager<short[]>	aacm;
	private int					filled = 0;
	private short[]				plain = null;
	private short[][]			sliced = null;

	private GrowableShortArray(final int initialPow, final int filled) {
		this.usePlain = true;
	...
	/**
	 * <p>Create growable short array instance</p>
	 ...
	 */
	public GrowableShortArray(final boolean usePlain) {
	 ...
	 */
	public GrowableShortArray(final boolean usePlain, final int initialPow) {
	 ...
	 * @return self
	 */
	public GrowableShortArray append(final short data) {
	 ...
	 */
	public GrowableShortArray append(final short[] data) throws NullPointerException {
		if (data == null) {
	 ...
	 */
	public GrowableShortArray append(final short[] data, int from, final int to) throws NullPointerException, ArrayIndexOutOfBoundsException {
	 ...
	 */
	public short read(final int index) throws ArrayIndexOutOfBoundsException {
	 ...
	 * @throws ArrayIndexOutOfBoundsException when index outside the data
	 */
	public int read(final int index, final short[] target) throws NullPointerException, ArrayIndexOutOfBoundsException {
		if (target == null) {
			throw new NullPointerException("Target data array can't be null");
		}
		else {
	 ...
	 * @throws ArrayIndexOutOfBoundsException when index outside the data
	 */
	public int read(int index, final short[] target, int from, final int to) throws NullPointerException, ArrayIndexOutOfBoundsException {
		if (target == null) {
			throw new NullPointerException("Target data array can't be null");
		}
		else if (filled == 0) {
	 ...
	 * @since 0.0.2
	 */
	public GrowableShortArray length(final int newLength) {
		if (newLength != filled) {
	 ...
	 */
	public GrowableShortArray toPlain() {
		if (usePlain) {
			return this;
		}
		else {
			final GrowableShortArray	result = new GrowableShortArray(initialPow,0);
	...
	public short[] toArray() {
		if (usePlain) {
			return plain == null ? NULL_SHORT : plain;
		}
		else {
	...
	public boolean equals(final Object obj) {
		...
		GrowableShortArray other = (GrowableShortArray) obj;
	...
	private class PlainManager extends AbstractPlainContentManager<short[]> {
		...
		int expandArray(final int newSize) {
			plain = plain == null ? new short[newSize] : Arrays.copyOf(plain, newSize);
			return newSize;
		}
...
//		@Override
//		void walk(Walker<short[]> walker) {
//			walker.process(plain,0,currentSize);
//		}
	}

	private class SlicedManager extends AbstractSlicedContentManager<short[]> {
		...
		@Override
		int expandArray(final int newSize) {
			sliced = sliced == null ? new short[newSize / initialSize][] : Arrays.copyOf(sliced,toSliceIndex(newSize));

			for (int index = sliced.length-1; index >= 0 && sliced[index] == null; index--) {
				sliced[index] = new short[initialSize];
			}
			return newSize;
		}
