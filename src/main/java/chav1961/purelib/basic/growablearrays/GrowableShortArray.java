...
public class GrowableShortArray {
	...
	private short[]				plain;
	private short[][]			sliced;

	private GrowableShortArray(final int initialPow, final int filled, final short[] data) {
		this.usePlain = true;
	 ...
	 * @return self
	 */
	public GrowableShortArray append(final short[] data, int from, final int to) {
	 ...
	 * @return data gotten
	 */
	public short read(final int index) {
	 ...
	 */
	public int read(final int index, final short[] target) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
		else {
	 ...
	 * @return real data size gotten
	 */
	public int read(int index, final short[] target, int from, final int to) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
	...
	public GrowableShortArray toPlain() {
		if (usePlain) {
			...
			final short[]	buffer = new short[len];
			...
			return new GrowableShortArray(initialSize,filled,buffer);
		}
	}
	...
	private void expandPlain(final int delta) {
		final short[]	newData = new short[2*plain.length];
	...
	private void internalClear() {
		if (this.usePlain) {
			this.sliced = null;
			this.plain = new short[this.initialSize];
		}
		else {
			this.sliced = new short[1][];
			this.sliced[0] = new short[this.initialSize];
		}
	}
