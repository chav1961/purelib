...
public class GrowableCharArray {
	...
	private GrowableCharArray(final int initialPow, final int filled, final char[] data) {
		this.usePlain = true;
	 ...
	 * @return self
	 */
	public GrowableCharArray append(final char[] data, int from, final int to) {
	 ...
	 * @return data gotten
	 */
	public char read(final int index) {
	 ...
	 */
	public int read(final int index, final char[] target) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
		else {
	 ...
	 * @return real data size gotten
	 */
	public int read(int index, final char[] target, int from, final int to) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
	...
	public GrowableCharArray toPlain() {
		if (usePlain) {
			...
			return new GrowableCharArray(initialSize,filled,buffer);
		}
	}
	...
	private void expandPlain(final int delta) {
		final char[]	newData = new char[2*plain.length];
	...
	private void internalClear() {
		if (this.usePlain) {
			this.sliced = null;
			this.plain = new char[this.initialSize];
		}
		else {
			this.sliced = new char[1][];
			this.sliced[0] = new char[this.initialSize];
		}
	}
