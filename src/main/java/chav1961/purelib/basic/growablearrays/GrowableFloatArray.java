...
public class GrowableFloatArray {
	...
	private float[]				plain;
	private float[][]			sliced;

	private GrowableFloatArray(final int initialPow, final int filled, final float[] data) {
		this.usePlain = true;
	 ...
	 * @return self
	 */
	public GrowableFloatArray append(final float[] data, int from, final int to) {
	 ...
	 * @return data gotten
	 */
	public float read(final int index) {
	 ...
	 */
	public int read(final int index, final float[] target) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
		else {
	 ...
	 * @return real data size gotten
	 */
	public int read(int index, final float[] target, int from, final int to) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
	...
	public GrowableFloatArray toPlain() {
		if (usePlain) {
			...
			final float[]	buffer = new float[len];
			...
			return new GrowableFloatArray(initialSize,filled,buffer);
		}
	}
	...
	private void expandPlain(final int delta) {
		final float[]	newData = new float[2*plain.length];
	...
	private void expandSliced(final int delta) {
		final int		expand = ((((filled & (initialSize-1)) + delta) + initialSize - 1) >> initialPow);
		final float[][]	newSliced = new float[sliced.length+expand][];
		
		System.arraycopy(sliced,0,newSliced,0,sliced.length);
		for (int index = sliced.length; index < newSliced.length; index++) {
			newSliced[index] = new float[initialSize];
		}
		sliced = newSliced;
	}
	...
	private void internalClear() {
		if (this.usePlain) {
			this.sliced = null;
			this.plain = new float[this.initialSize];
		}
		else {
			this.sliced = new float[1][];
			this.sliced[0] = new float[this.initialSize];
		}
	}
