...
public class GrowableFloatArray {
	...
	private final AbstractArrayContentManager<float[]>	aacm;
	private int					filled = 0;
	private float[]				plain = null;
	private float[][]			sliced = null;

	private GrowableFloatArray(final int initialPow, final int filled) {
		this.usePlain = true;
	...
	/**
	 * <p>Create growable float array instance</p>
	 ...
	 */
	public GrowableFloatArray(final boolean usePlain) {
	 ...
	 */
	public GrowableFloatArray(final boolean usePlain, final int initialPow) {
	 ...
	 * @return self
	 */
	public GrowableFloatArray append(final float data) {
	 ...
	 */
	public GrowableFloatArray append(final float[] data) throws NullPointerException {
		if (data == null) {
	 ...
	 */
	public GrowableFloatArray append(final float[] data, int from, final int to) throws NullPointerException, ArrayIndexOutOfBoundsException {
	 ...
	 */
	public float read(final int index) throws ArrayIndexOutOfBoundsException {
	 ...
	 * @throws ArrayIndexOutOfBoundsException when index outside the data
	 */
	public int read(final int index, final float[] target) throws NullPointerException, ArrayIndexOutOfBoundsException {
		if (target == null) {
			throw new NullPointerException("Target data array can't be null");
		}
		else {
	 ...
	 */
	public int read(int index, final float[] target, int from, final int to) throws NullPointerException, ArrayIndexOutOfBoundsException {
		if (target == null) {
			throw new NullPointerException("Target data array can't be null");
		}
		else if (filled == 0) {
	 ...
	 * @since 0.0.2
	 */
	public GrowableFloatArray length(final int newLength) {
		if (newLength != filled) {
	 ...
	 */
	public GrowableFloatArray toPlain() {
		if (usePlain) {
			return this;
		}
		else {
			final GrowableFloatArray	result = new GrowableFloatArray(initialPow,0);
	...
	public float[] toArray() {
		if (usePlain) {
			return plain == null ? NULL_FLOAT : plain;
		}
		else {
	...
	public String toString() {
		return "GrowableFloatArray [usePlain=" + usePlain + ", initialSize=" + initialSize + ", filled=" + filled + ", plain=" + Arrays.toString(plain) + ", sliced=" + Arrays.toString(sliced) + "]";
	}
	...
	public boolean equals(final Object obj) {
		...
		GrowableFloatArray other = (GrowableFloatArray) obj;
	...
	private class PlainManager extends AbstractPlainContentManager<float[]> {
		...
		int expandArray(final int newSize) {
			plain = plain == null ? new float[newSize] : Arrays.copyOf(plain, newSize);
			return newSize;
		}
...
//		@Override
//		void walk(Walker<float[]> walker) {
//			walker.process(plain,0,currentSize);
//		}
	}

	private class SlicedManager extends AbstractSlicedContentManager<float[]> {
		...
		@Override
		int expandArray(final int newSize) {
			sliced = sliced == null ? new float[newSize / initialSize][] : Arrays.copyOf(sliced,toSliceIndex(newSize));

			for (int index = sliced.length-1; index >= 0 && sliced[index] == null; index--) {
				sliced[index] = new float[initialSize];
			}
			return newSize;
		}
