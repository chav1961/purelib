...
public class GrowableBooleanArray {
	...
	private GrowableBooleanArray(final int initialPow, final int filled, final boolean[] data) {
		this.usePlain = true;
	 ...
	 * @return self
	 */
	public GrowableBooleanArray append(final boolean[] data, int from, final int to) {
	 ...
	 */
	public int read(final int index, final boolean[] target) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
		else {
	 ...
	 * @return real data size gotten
	 */
	public int read(int index, final boolean[] target, int from, final int to) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
	...
	public GrowableBooleanArray toPlain() {
		if (usePlain) {
			...
			final boolean[]	buffer = new boolean[len];
			...
			return new GrowableBooleanArray(initialSize,filled,buffer);
		}
	}
	...
	private void expandPlain(final int delta) {
		final boolean[]	newData = new boolean[2*plain.length];
	...
	private void internalClear() {
		if (this.usePlain) {
			this.sliced = null;
			this.plain = new boolean[this.initialSize];
		}
		else {
			this.sliced = new boolean[1][];
			this.sliced[0] = new boolean[this.initialSize];
		}
	}
