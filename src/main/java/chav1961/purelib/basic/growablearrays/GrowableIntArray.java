...
public class GrowableIntArray {
	...
	private GrowableIntArray(final int initialPow, final int filled, final int[] data) {
		this.usePlain = true;
	 ...
	 * @return self
	 */
	public GrowableIntArray append(final int[] data, int from, final int to) {
	 ...
	 */
	public int read(final int index, final int[] target) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
		else {
	 ...
	 * @return real data size gotten
	 */
	public int read(int index, final int[] target, int from, final int to) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
	...
	public GrowableIntArray toPlain() {
		if (usePlain) {
			...
			return new GrowableIntArray(initialSize,filled,buffer);
		}
	}
	...
	private void expandPlain(final int delta) {
		final int[]	newData = new int[2*plain.length];
	...
	private void internalClear() {
		if (this.usePlain) {
			this.sliced = null;
			this.plain = new int[this.initialSize];
		}
		else {
			this.sliced = new int[1][];
			this.sliced[0] = new int[this.initialSize];
		}
	}
