...
public class GrowableLongArray {
	...
	private GrowableLongArray(final int initialPow, final int filled, final long[] data) {
		this.usePlain = true;
	 ...
	 * @return self
	 */
	public GrowableLongArray append(final long[] data, int from, final int to) {
	 ...
	 * @return data gotten
	 */
	public long read(final int index) {
	 ...
	 */
	public int read(final int index, final long[] target) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
		else {
	 ...
	 * @return real data size gotten
	 */
	public int read(int index, final long[] target, int from, final int to) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
	...
	public GrowableLongArray toPlain() {
		if (usePlain) {
			...
			final long[]	buffer = new long[len];
			...
			return new GrowableLongArray(initialSize,filled,buffer);
		}
	}
	...
	private void expandPlain(final int delta) {
		final long[]	newData = new long[2*plain.length];
	...
	private void internalClear() {
		if (this.usePlain) {
			this.sliced = null;
			this.plain = new long[this.initialSize];
		}
		else {
			this.sliced = new long[1][];
			this.sliced[0] = new long[this.initialSize];
		}
	}
