...
/**
 * <p>This class implements functionality for the growable ‚˘„Ë‰Û arrays.</p>
...
public class GrowableDoubleArray {
	...
	private double[][]			sliced;

	private GrowableDoubleArray(final int initialPow, final int filled, final double[] data) {
		this.usePlain = true;
	 ...
	 * @return self
	 */
	public GrowableDoubleArray append(final double[] data, int from, final int to) {
	 ...
	 */
	public int read(final int index, final double[] target) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
		else {
	 ...
	 * @return real data size gotten
	 */
	public int read(int index, final double[] target, int from, final int to) {
		if (target == null) {
			throw new IllegalArgumentException("Target data array can't be null");
		}
	...
	public GrowableDoubleArray toPlain() {
		if (usePlain) {
			...
			return new GrowableDoubleArray(initialSize,filled,buffer);
		}
	}
	...
	private void expandPlain(final int delta) {
		final double[]	newData = new double[2*plain.length];
	...
	private void internalClear() {
		if (this.usePlain) {
			this.sliced = null;
			this.plain = new double[this.initialSize];
		}
		else {
			this.sliced = new double[1][];
			this.sliced[0] = new double[this.initialSize];
		}
	}
