...
public class StringEditor<R> implements SwingItemEditor<String, R> {
	...
	@Override
	public boolean canServe(final Class<String> class2Edit, final Class<R> editorType, final Object... options) {
		if (class2Edit == null) {
			...
			return canServe((Class<String>) class2Edit.getComponentType(), editorType, options);
		}
		else {
			return String.class.isAssignableFrom(class2Edit) && SUPPORTED_EDITORS.contains(editorType); 
		}
	}
	...
	public R getEditor(final Class<R> editorType, final Object... options) {
		if (editorType == null) {
			...
			try{final Class<?>				clazz = options.length > 0 && (options[0] instanceof Class) ? (Class<?>)options[0] : String.class;
