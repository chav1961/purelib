...
import chav1961.purelib.ui.nanoservice.interfaces.HeaderName;
import chav1961.purelib.ui.nanoservice.interfaces.MethodExecutor;
import chav1961.purelib.ui.nanoservice.interfaces.NanoService;
...
public class NanoServiceFactory implements Closeable, NanoService, HttpHandler   {
	...
	public static final String 		TST_SERVER = "keys/servertrust.jks";
	...
	public interface MicroServiceEnvironment {
	...
	private final MicroServiceEnvironment			environment;
	...
	public NanoServiceFactory(final LoggerFacade facade, final SubstitutableProperties props, final DataSource dataSource) throws NullPointerException, IOException, ContentException, SyntaxException {
		if (facade == null) {
			...
			try(final LoggerFacade	check = facade.transaction("Microservice init")) {
				...
				if (!wereErrors) {
					this.environment = new MicroServiceEnvironment() {
						@Override
					...
					try(final InputStream	is = NanoServiceFactory.class.getResourceAsStream("macros.txt");
...
//			}
	...
	protected MicroServiceEnvironment getEnvironment() {
		return environment;
	}
	...
	private static SSLContext createSSLContext(final SubstitutableProperties props) throws IOException  {
	    try{final SSLContext			sslContext = SSLContext.getInstance("TLS");
	        ...
	        try(final InputStream 	is = new FileInputStream(KST_SERVER)) {
	        	ks.load(is,SERVER_PWD.toCharArray());
	        }
	        try(final InputStream 	is = new FileInputStream(TST_SERVER)) {
	        	ts.load(is, SERVER_PWD.toCharArray());
	        }
	...
	ClassDescriptor parseClass(final Object instance, final Class<?> clazz) throws ContentException, SyntaxException {
	...
	MethodExecutor buildMethodExecutor(final Object instance, final Method m, final String value, final QueryType[] type, final MimeType[] source, final MimeType[] target) throws ContentException, SyntaxException {
		for (Parameter item : m.getParameters()) {
			int	count = 0;
	...
	MethodExecutor buildGetMethodExecutor(final Object instance, final Method m, final String path, final MimeType[] source, final MimeType[] target) throws SyntaxException, ContentException {
	...
	MethodExecutor buildPostMethodExecutor(final Object instance, final Method m, final String path, final MimeType[] source, final MimeType[] target) throws SyntaxException, ContentException {
		final String[]		pathNames = collectPathParameters(m)
							, requestHeaderNames = collectRequestParameters(m)
							, responseHeaderNames = collectResponseParameters(m);
		final AnyKindParserAndSetter forPath = pathNames.length > 0 ? findOrBuildPathParser(path.startsWith("/") ? path.substring(1) : path) : null
	...
	MethodExecutor buildPutMethodExecutor(final Object instance, final Method m, final String path, final MimeType[] source, final MimeType[] target) throws SyntaxException, ContentException {
		final String[]		pathNames = collectPathParameters(m)
							, requestHeaderNames = collectRequestParameters(m)
							, responseHeaderNames = collectResponseParameters(m);
		final AnyKindParserAndSetter forPath = pathNames.length > 0 ? findOrBuildPathParser(path.startsWith("/") ? path.substring(1) : path) : null
	...
	MethodExecutor buildDeleteMethodExecutor(final Object instance, final Method m, final String path, final MimeType[] source, final MimeType[] target) throws SyntaxException, ContentException {
		final String[]		pathNames = collectPathParameters(m)
							, requestHeaderNames = collectRequestParameters(m)
							, responseHeaderNames = collectResponseParameters(m);
		final AnyKindParserAndSetter forPath = pathNames.length > 0 ? findOrBuildPathParser(path.startsWith("/") ? path.substring(1) : path) : null
	...
	private static String collectAdditionalImports(final Method m) throws ContentException {
