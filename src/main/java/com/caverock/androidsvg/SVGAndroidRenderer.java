...
class SVGAndroidRenderer
{
   ...
   private class RendererState
   {
      ...
      RendererState(RendererState copy)
      {
         ...
         {
            // Should never happen
            System.err.println("Unexpected clone error"+e);
            style = Style.getDefaultStyle();
         }
      }
   }
   ...
   void  renderDocument(SVG document, RenderOptions renderOptions)
   {
      ...
      if (renderOptions.hasView())
      {
         ...
         if (obj == null || !(obj instanceof SVG.View)) {
            System.err.println(String.format("View element with id \"%s\" not found."+renderOptions.viewId));
            return;
         }
         SVG.View  view = (SVG.View) obj;

         if (view.viewBox == null) {
        	 System.err.println(String.format("View element with id \"%s\" is missing a viewBox attribute."+renderOptions.viewId));
            return;
         }
...
//private Object getFillTypeFromState() {
...
private Box  calculatePathBounds(Path path)
   ...
   private Bitmap  checkForImageDataURL(String url)
   {
      ...
      if (!";base64".equals(url.substring(comma-7, comma)))
         return null;
      try {
         byte[]  imageData = Base64.getDecoder().decode(url.substring(comma+1));  // throws IllegalArgumentException for bad data
         return BitmapFactory.decodeByteArray(imageData, 0, imageData.length);
      } catch (Exception e) {
         System.err.println("Could not decode bad Data URL"+e);
         return null;
      }
   }
