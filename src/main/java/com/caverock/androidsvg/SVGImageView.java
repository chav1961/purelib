...
/**
   ...
   {
      if (!internalSetImageURI(uri))
         System.err.println("SVGImageView"+"File not found: " + uri);
   }
   ...
   public void  setImageAsset(String filename)
   {
      if (!internalSetImageAsset(filename))
    	  System.err.println("SVGImageView"+"File not found: " + filename);
   }
   ...
   {
      try {
         this.svg = SVG.build(url);
         doRender();
      } catch (SVGParseException e) {
         // Failed to interpret url as a resource, a filename, or an actual SVG...
         System.err.println("SVGImageView"+"Could not find SVG at: " + url);
      }
   }
	...
	protected SVG  doInBackground(Integer... params)
      {
         ...
         {
            System.err.println("SVGImageView"+String.format("Error loading resource 0x%x: %s", resourceId, e.getMessage()));
         }
         return null;
      }
   ...
   private class LoadURITask extends AsyncTask<InputStream, Integer, SVG>
   {
      protected SVG  doInBackground(InputStream... is)
      {
         try
         {
            return SVG.build(is[0]);
         }
         catch (SVGParseException e)
         {
            System.err.println("SVGImageView"+"Parse error loading URI: " + e.getMessage());
         }
         finally
         {
            try
   ...
   private void  setSoftwareLayerType()
   {
      ...
      {
         System.err.println("SVGImageView"+"Unexpected failure calling setLayerType"+e);
      }
   }
...
private void setImageDrawable(PictureDrawable pictureDrawable) {
	// TODO Auto-generated method stub
