...
class SVGParser
{
   ...
   private static class ColourKeywords {
		private static class ColorKeyword implements Comparable<ColorKeyword>{
			private final String	key;
			private final int		value;
					
			private ColorKeyword(final String key, final int value) {
				this.key = key;
				this.value = value;
			}
			
			@Override
			public int compareTo(final ColorKeyword o) {
				return key.compareTo(o.key);
			}
			
			@Override
			public String toString() {
				return "ColorKeyword [key=" + key + ", value=" + value + "]";
			}
		}
		
		private static final ColorKeyword[]	COLORS;
      
		static {
		   final List<ColorKeyword>	temp = new ArrayList<>();
		   final int[]				content = new int[1];
		   
		   try(final LineByLineProcessor	lblp = new LineByLineProcessor(
				   (displacement,lineNo,data,from,length)->{
					   for (int index = 0; index < length; index++) {
						   if (data[index] == ',') {
							   CharUtils.parseInt(data,index+1,content,true);
							   temp.add(new ColorKeyword(new String(data,from,index-1),content[0]));
						   }
					   }
				   });
			   final InputStream			is = ColourKeywords.class.getResourceAsStream("colorpair.csv");
		   ...
		   COLORS = temp.toArray(new ColorKeyword[temp.size()]);
		   Arrays.sort(COLORS);
		   temp.clear();
		}

		static int get(final String colorName) {
	        int low = 0, high = COLORS.length - 1, mid, comp;

	        while (low <= high) {
	            mid = (low + high) >>> 1;
	            comp = colorName.compareTo(COLORS[mid].key);
   ...
   SVG  parse(InputStream is, boolean enableInternalEntities) throws SVGParseException
   {
      ...
      finally
      {
         try {
            ...
            System.err.println("Exception thrown closing input stream");
         }
      }
      return svgDocument;
   }
            ...
            switch(eventType) {
               ...
               case XmlPullParser.DOCDECL:
                  if (enableInternalEntities &&                  // entities are enabled
                     ...
                     try {
                        System.err.println("Switching to SAX parser to process entities");
                        is.reset();
                        parseUsingSAX(is);
                     } catch (IOException e) {
                        // reset() failed
                    	 System.err.println("Detected internal entity definitions, but could not parse them.");
               ...
               case XmlPullParser.PROCESSING_INSTRUCTION:
            	  System.err.println("PROC INSTR: "+parser.getText());
...
//      }
   ...
   {
      System.err.println("Falling back to SAX parser");
      try
      {
      ...
      while (true)
      {
         ...
         switch (pathCommand)
         {
            ...
            case 'm':
               ...
               if (Float.isNaN(y)) {
            	   System.err.println("Bad path coords for "+((char)pathCommand)+" path segment");
                  return path;
               }
